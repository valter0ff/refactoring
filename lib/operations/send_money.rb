class SendMoney < MoneyOperations
  ...
  def choose_card_message
    puts I18n.t('common.choose_card_for_sending')
  end

  def set_recipient_card
    puts I18n.t('common.enter_recipient_card')
    answer = gets.chomp
    return puts I18n.t('errors.incorrect_card_number') if answer.size != 16

    recipient_card = @accounts.flat_map(&:cards).find { |card| card.number == answer }
    return puts I18n.t('errors.no_such_card', card_number: answer) if recipient_card.nil?
